...
Per goal: app sets `config` to the ESLint config, `runner` to the canonical `npx eslint .` (one
pinned form, never `eslint apps`, since the flat config's `files` glob already scopes it), and
