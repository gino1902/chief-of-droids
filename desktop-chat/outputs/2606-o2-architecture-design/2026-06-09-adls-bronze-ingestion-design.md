...
Items 1 to 4 are runtime and ingestion checks. Items 5 to 9 are the storage and integrity security baseline, which sits in the shared head and applies to every option equally regardless of the option chosen.
...
5. Lock write access to the landing zone to the extractor identity alone, so no other principal can land a file in the first place.
6. Put the storage account on a private network, no public access, with firewall rules and private endpoints.
7. Enforce encryption at rest, platform or customer-managed keys, and in transit.
8. Validate content before the MERGE into bronze, schema and contract checks, size and type limits, and quarantine anomalies. Treat unstructured or binary uploads as untrusted.
9. Trigger ingestion only on the real commit event (FlushWithClose) from the trusted account, so a forged or premature event cannot inject data.
...
Two preconditions sit above the scoring. The platform mandates Unity Catalog governance, so an out-of-UC path is admissible only by explicit exception. And the storage and integrity security baseline in standing checks 5 to 9 applies to every option equally, so it is not scored here.
