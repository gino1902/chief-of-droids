...
> Locked on Auto Loader with managed file events.
...
The ingestion path is Auto Loader with managed file events, on a Unity Catalog external location. The run is launched by a Lakeflow job file-arrival trigger and drains with `Trigger.availableNow`. This is event-driven launch plus batch drain: the cluster runs only when new files have arrived, so there is no idle compute and no fixed schedule interval. A scheduled `Trigger.availableNow` batch and a continuous stream are the two fallbacks, the scheduled batch where a fixed cadence is preferred, the continuous stream only if a sub-minute freshness SLA is set. The deciding inputs are the extractor's pull cadence and the bronze freshness target.

Three facts underpin the decision:
...
- On the managed path the file events service provisions and owns the Event Grid subscription and storage queue, one shared pair per external location [5][13]. You do not configure that subscription, and file events keys ingestion on the `FlushWithClose` commit event [11]. The duplication, delivery and bring-your-own-queue detail sits in checks 3 and 13.

## Solution benchmark

The locked solution scored against the platform selection criteria. Ratings are 1 to 5 on how well it satisfies each criterion (5 best), weighted by the platform criterion weight.

| # | Criterion | Weight | Rating | Weighted | Rationale |
| :--- | :--- | ---: | ---: | ---: | :--- |
| 1 | Compute cost impact | 5 | 4 | 20 | A continuous stream bills idle between pulls. File-arrival-triggered availableNow runs the cluster only when new files arrive, so idle compute is removed |
| 2 | Latency, event to bronze | 3 | 4 | 12 | Continuous mode has the lowest latency via the cache hop with no per-run cluster start. File-arrival launch removes the fixed schedule interval but adds about a minute of best-effort trigger evaluation plus cluster start [11] |
...
| 6 | Maturity and longevity | 3 | 4 | 12 | Managed file events and file-arrival triggers are documented GA mechanisms on a current runtime |
...
Verdict: the locked configuration, a file-arrival trigger launching an availableNow drain, is the strongest fit for a periodic pull extractor. The cluster runs only when files arrive, so idle compute is removed without going continuous, while the operational-ownership and security lead the profile shows is retained. The fallbacks are a scheduled availableNow batch where a fixed cadence is preferred, and a continuous stream only if a sub-minute freshness SLA appears. The pull cadence and the bronze freshness target remain the inputs that finalise the configuration, and the trigger's minimum-time-between-runs and wait-after-last-change settings [11] are the levers that reconcile them.
...
| 2. Land file to ADLS Gen2 | ADLS Gen2 (HNS), abfss driver, CreateFile + FlushWithClose, unique filenames, cloudFiles.cleanSource (DBR 16.4 LTS+) | Durable landing, FlushWithClose gives a clean commit boundary, cleanSource caps source-directory storage cost | Same-name overwrite does not retrigger discovery, so keep unique filenames (check 4). Discovery is incremental, so cleanSource is a storage-cost lever, not a discovery-speed one | [1][3][8] |
| 3. Emit event, ADLS to the file events queue | Event Grid subscription + Azure Storage Queue. On the managed path the file events service provisions and owns this pair, one per external location [13]. Microsoft.Storage.BlobCreated (HNS fires on both CreateFile and FlushWithClose) | Native push, no polling, no resources for you to operate on the managed path | Service-owned subscription, ingestion keyed on FlushWithClose [11], at-least-once delivery so duplicates are possible. See checks 3 and 13 | [2][3][5][11][13] |
| 4. Catch and ingest, managed file events | Auto Loader stream, useManagedFileEvents=true (automatic on DBR 18.1+), one managed queue per UC external location, per-subpath volume, reads cache for discovery and storage for bytes, launched by a Lakeflow job file-arrival trigger, Trigger.availableNow drain, RocksDB checkpoint exactly-once | Fewest moving parts, one queue per location, no extra creds, managed tuning and cleanup, default-on, exactly-once, the cluster runs only on arrival | Cache-hop latency, 7-day re-listing fallback, 24h reconciliation scan, per-subpath volume, immutable files plus the trigger paired with Auto Loader (both met here), batching via minimum-time-between-runs and wait-after-last-change [11]. See checks 6 and 7, notes 2 and 3 | [4][5][6][11][12] |
| 5. Write to bronze as VARIANT | Delta bronze table, singleVariantColumn whole-record VARIANT (DBR 15.3+), promoted columns (ingest_ts, source_path, business_key) | Schema-flexible semi-structured storage, replaces JSON strings, queryable | VARIANT cannot be a partition, clustering or Z-order key nor used in compare, group or order. 16 MB record cap. Public Preview. Promote keys and match casing exactly, see check 5 | [9][10] |
...
3. The FlushWithClose boundary is handled by the managed path. The subscription is service-owned and file events processes a file on the FlushWithClose commit event [11], so you do not need to enforce the filter yourself. The residual risk is delayed discovery where an Azure API write pattern omits FlushWithClose [11], not premature ingestion. Keep the ingest idempotent regardless. Adopt the bring-your-own-queue variant only if you must own the subscription and its filter directly. Confirm HNS event behaviour against the File events FAQ [F].
...
6. Scope discovery to a per-subpath Unity Catalog volume rather than the bare external location, to avoid the Too many requests rate limit when several streams read different subpaths under one location. The same per-subpath volume also keeps a file-arrival trigger from timing out on extraneous changes elsewhere under the location [11].
7. Make sure the run fires at least once every 7 days, or the stored read position expires and Auto Loader falls back to a full directory listing. A file-arrival-triggered or scheduled availableNow run satisfies this only while it fires that often. A location that can be quiet for more than 7 days should add a low-frequency scheduled safety run alongside the file-arrival trigger.
...
| CreateFile / FlushWithClose | The Data Lake Gen2 write operations. FlushWithClose marks the file fully committed, the only safe trigger boundary. Managed file events processes a file on FlushWithClose [11] |
...
| File events (managed) | A Databricks service that provisions and owns the Event Grid subscription and storage queue (one shared pair per external location), listens to storage notifications, caches file metadata, and serves two consumers: Auto Loader discovery and the Lakeflow job file-arrival trigger [5][11] |
| File-arrival trigger | A Lakeflow job trigger that launches a run when new files arrive in a Unity Catalog external location or volume. On a file-events-enabled location it uses the managed file events service, checks about every minute on a best-effort basis, and supports minimum-time-between-runs and wait-after-last-change batching controls [11] |
...
| Trigger.availableNow | A batch trigger that processes all files present at start, then stops. The locked drain mode, launched by the file-arrival trigger |
...
| Lakeflow job | The Databricks job that runs the Auto Loader stream, launched by a file-arrival trigger |
...
| 11 | Trigger jobs when new files arrive | [learn.microsoft.com](https://learn.microsoft.com/en-us/azure/databricks/jobs/file-arrival-triggers) |
| 12 | Automate jobs with schedules and triggers | [learn.microsoft.com](https://learn.microsoft.com/en-us/azure/databricks/jobs/triggers) |
| 13 | Manage external locations (file events, Event Grid + Storage Queue) | [learn.microsoft.com](https://learn.microsoft.com/en-us/azure/databricks/connect/unity-catalog/cloud-storage/manage-external-locations) |
