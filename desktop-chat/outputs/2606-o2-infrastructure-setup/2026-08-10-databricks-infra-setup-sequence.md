...
sequenceDiagram
    ...
    and
        ...
        PE->>AZ: Provision VNet (/16 to /24), host and container subnets (/26 floor, /23 typical), each delegated to Microsoft.Databricks/workspaces with an NSG attached, SCC, NAT gateway on the host subnet (both subnets if stable egress IPs are needed)
...
- VNet injection has two subnet prerequisites that are easy to miss, because the network reads as complete without them. Both subnets need a delegation to `Microsoft.Databricks/workspaces`, and both need a network security group attached. Leave that NSG empty. Databricks auto-provisions and manages the rules it needs through the delegation, and the docs tell you not to modify or delete them. The portal configures both automatically, which is why this catches people deploying by Terraform. The workspace deployment fails against undelegated subnets, and the network looks correct right up to that point.
...
- Three identity features are on without being switched on here, but they are three separate defaults with different scopes and dates, not one blanket new-account rule. Identity federation is on for all new workspaces and cannot be disabled, and it is scoped to workspaces rather than accounts. Automatic identity management is on by default for accounts created after 1 August 2025, which covers this account. JIT provisioning is on by default for accounts created after 1 May 2025, and it is separately forced on and locked whenever automatic identity management is enabled, so it is on here twice over. The account admin verifies all three rather than enabling them. Do not generalise this: an account created between those two dates gets JIT without automatic identity management, and an older account gets neither. There is no SSO step on Azure: Entra-backed login is on by default for both the account console and workspaces, for all customers. The Phase 1 page's generic advice to authenticate via SSO with your identity provider is cross-cloud wording and does not describe an Azure action.
...
- Automatic identity management, default on for accounts created after 1 August 2025, Entra as source of record: https://learn.microsoft.com/en-us/azure/databricks/admin/users-groups/automatic-identity-management/
- JIT provisioning, default on for accounts created after 1 May 2025, forced on and locked under automatic identity management: https://learn.microsoft.com/en-us/azure/databricks/security/auth/jit
- Identity federation, default on for all new workspaces and not disableable: https://learn.microsoft.com/en-us/azure/databricks/admin/users-groups/
...
- VNet injection, two subnets at least /26, VNet /16 to /24, NAT gateway required for egress since 31 March 2026, delegation to Microsoft.Databricks/workspaces and an NSG required on both subnets, auto-provisioned NSG rules: https://learn.microsoft.com/en-us/azure/databricks/security/network/classic/vnet-inject
...
<!--
Version: 1.12 | Last Updated: 2026-08-10 | Status: Draft
-->
