...
- The Azure Databricks account is created on the Premium tier.
...
> Produce and sign off the solution architecture for the Azure Databricks platform, covering topology, networking, identity, storage, and the IaC and CI/CD approach, so that infrastructure and delivery decisions are agreed before build and rework is avoided. For this greenfield account, new workspaces are enabled for Unity Catalog automatically with a regional metastore auto-provisioned (manual metastore creation is deprecated), and automatic identity management is the default for accounts created after 1 August 2025. Depends on Epic 1.

Acceptance criteria:
- Target topology is documented: workspaces, regions, and Unity Catalog enablement per region.
...
- Identity design is decided: Microsoft Entra ID as the identity provider with automatic identity management, and the service principal model for automation.
...
> Deploy the Azure plumbing for the platform: the Azure Databricks workspace, network isolation, ADLS Gen2 storage with its access connector, Unity Catalog enablement, and Entra ID identity wiring, so that a secure, network-isolated platform is ready to host governed workloads. Premium is required because Unity Catalog only attaches Premium workspaces, and the regional metastore is auto-provisioned for new workspaces (since 9 November 2023), so the manual create-metastore step is deprecated and not run. Depends on Epics 1 and 2.

Acceptance criteria:
- The Azure Databricks workspace is deployed on the Premium tier via ARM or Bicep, not manual portal clicks.
...
- The workspace is automatically enabled for Unity Catalog, and the storage credential is registered.
...
> Establish version-controlled, repeatable deployment for all Databricks code and resources using Declarative Automation Bundles and a service-principal-authenticated Azure DevOps pipeline, so that every change after this point ships as a reviewable, automated release across environments. Azure DevOps is chosen over GitHub Actions for ecosystem compliance and integration with existing Azure DevOps tooling. Authentication uses an Azure Resource Manager service connection: workload identity federation (OIDC) is the recommended secret-free method, with service-principal client secret and managed identity as the alternatives. Depends on Epic 3, and is the delivery mechanism for Epics 5 to 8.
...
- An Azure Pipelines (Azure DevOps) pipeline runs `databricks bundle deploy`.
- Deployment authenticates as a Microsoft Entra service principal through an Azure Resource Manager service connection.
...
- Microsoft Entra ID with automatic identity management syncs users and groups from the identity provider.
- A synced group is assigned to the workspace, and its members gain workspace access only after that assignment.
...
- Policy-level filtering uses `usage_metadata.usage_policy_id`.

## Epic 8 (Business) — Usage attribution for ingestion and consumption

> Build usage monitoring that attributes ingestion (Auto Loader, Lakeflow Spark Declarative Pipelines, Lakeflow Jobs) and consumption (SQL warehouse queries, Model Serving model and agent endpoints) to an identity, using `system.billing.usage` `identity_metadata` and correlating control-plane actor data from `system.access.audit`, so that DBUs are traceable to a user or service principal (agent) for the workload types that expose identity. Identity resolution is best-effort: `identity_metadata` populates only for certain workload types, and `system.access.audit` names control-plane actors but holds no usage quantities, so it corroborates an identity rather than attributing billed usage on its own. Depends on Epics 5, 6 and 7. PI2 candidate.
...
- Usage without an identity field (for example all-purpose compute or plain model-serving endpoints) is flagged as unattributable rather than forced onto an owner.
...
<!--
Version: 1.11 | Last Updated: 2026-06-26 | Status: Draft
-->
