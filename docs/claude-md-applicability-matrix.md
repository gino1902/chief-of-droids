...
- **Chat** — Claude Desktop chat (claude.ai web app or desktop client). Two configuration slots per Project: Project Instructions (applies to every chat in the project) and Project Knowledge (uploaded files, RAG-scaled). No native hierarchical instruction-file mechanism.
...
Verified 2026-05-13 (second pass) against fetched content.
...
| 18 | Claude Desktop Projects expose exactly two slots: Project Instructions (one per project, applies to every chat in the project) and Project Knowledge (uploaded files, RAG-scaled) | ✅ | ✅ | ❌ |
...
### Verified column — claim provenance

| Symbol | Meaning | Operational reading |
|---|---|---|
| ✅ | Official | Direct quote or close paraphrase from an Anthropic published source (Claude Code docs at `code.claude.com`, support articles at `support.claude.com`). Citable as official. |
| 🔶 | Inferred | Derived governance or cognitive principle, soundly extrapolated from verified mechanics but not stated in any official source. Use with judgment; do not cite as official. |
| 🛠 | Bespoke | Verified against workspace state (this workspace's `CLAUDE.md`, `HOW-TO-TRIGGER.md`, bootstrap mechanism). Translation claim — not in any Anthropic source because it describes the workspace's own adaptation. |

### Chat / Code columns — runtime applicability
