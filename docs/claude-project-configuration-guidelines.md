...
> `.claude/skills/name/SKILL.md`. Both create `/name` and work the same way
> (`code.claude.com/docs/en/skills`, verified 2026-05-29); the docs do not
> specify collision precedence. Prefer skills for new work, they support
> supporting files, frontmatter, and
...
- Target **≤ 200 lines**. If you exceed, split into `.claude/rules/` files, skills, or imports before writing.
- Markdown headers + bullets. No dense paragraphs.
- Markdown headers and bullets (above) are the structure the Claude Code memory and best-practices docs recommend
  (`code.claude.com/docs/en/best-practices`, verified 2026-05-29). One level of header nesting is usually enough.
- No emoji in the file. The best-practices doc bans decorative formatting, including emoji. Use plain-text markers for scaffolding (for example "(planned, not yet authored)" or "TBD"), not flag glyphs.
- XML-tagged rule groups (`<safety_rules>`, etc.) are optional, not required. They come from general Claude API prompt-engineering guidance (`docs.claude.com/en/docs/build-with-claude/prompt-engineering/use-xml-tags`), which is not part of the Claude Code CLAUDE.md documentation. Claude parses them, but prefer markdown headers unless a framing explicitly calls for tags.
- Every instruction must be specific and verifiable: write `Run npm test before committing`, not `Test your changes`.
- All discovered memory files are concatenated, ordered from the filesystem root down to the working directory (`CLAUDE.local.md` is appended after `CLAUDE.md` within a directory). Content closer to cwd appears later, which biases resolution but does not hard-override earlier content. How Claude resolves a direct contradiction still depends on instruction specificity, so fix contradictions before write rather than relying on order. Source: `code.claude.com/docs/en/memory`, verified 2026-05-29.
- Open the file with an HTML comment recording: framing source, generation date, guidelines version. Costs zero tokens.
- After writing, run `/memory` to confirm every CLAUDE.md / CLAUDE.local.md / rules file you authored is discovered and loaded.
...
- Relative paths resolve to the importing file's location, not the working
  directory.
- Max 4 hops (recursive import depth). Verified 2026-05-29 against
