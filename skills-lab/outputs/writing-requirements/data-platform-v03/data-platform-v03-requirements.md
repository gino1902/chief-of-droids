...
- Access control to the Data Platform via Active Directory; access control at data-asset level per RGPD and SQLI data access policy.
- Encryption of data in transit across external trust boundaries.
- Observability of incoming and outgoing data flows; production and consumption traceability (who, when, where).
...
> ⚠️ Out of Scope: substrate provides no explicit out-of-scope signals. Items marked **TBD** or "in design phase" (F8, F17, NFR1, NFR2, NFR6, NFR7) are deferred decisions, not exclusions.
...
> ⚠️ Specific subsystem identities, named operator roles, and named business-user roles are not enumerated in substrate; rows describe abstract roles only.
...
| Data catalogue | An indexed inventory of all the data the company holds. Lets users find what data exists, where it lives, and who owns it. | substrate |
| Data classification | Labelling data by sensitivity (public, internal, confidential, personal). The label determines who can access it and how it must be protected. | substrate |
...
| Dead-letter | A holding area for messages or records that failed to process and could not be retried. Lets engineers inspect failures rather than lose them silently. | substrate |
...
| Raw ingestion | Loading data into the platform exactly as it arrived, with no cleaning or transformation, so the original is preserved. | substrate |
...
| SaaS (Software as a Service) | Software hosted by a vendor and accessed over the internet (e.g. Salesforce, Workday). | substrate |
...
| Subsystem | A self-contained part of the wider system, with its own job and boundary (e.g. a CRM application, the data warehouse). | substrate |
| Subsystem-agnostic format | A storage format chosen so that no single subsystem's technology is required to read it — any authorised subsystem can consume it. | substrate |
...
**FR-008** — The Data Platform SHALL expose business-ready, standardised data for direct consumption by authorised consuming subsystems.
...
**FR-011** — The Landing Zone SHALL retain ingested data for 7 days from ingestion.
...
> ⚠️ FR-008 and FR-014: substrate marks "use cases to be defined in the future" — exposure obligation is rendered; downstream use-case enumeration is deferred.
...
**IR-OUT-004** — The System MUST expose observability signals (data flow events, production and consumption traces, usage and cost telemetry, audit records) to authorised monitoring consumers.
...
**NFR-002** — The System SHOULD route records that fail processing after retry exhaustion to a dead-letter store. | Measurement: TBD — dead-letter retention and triage SLA to be defined.
...
**NFR-005** — The System SHOULD meet defined data freshness and latency SLOs per dataset. | Measurement: TBD with Business Users in design phase.
...
**SEC-004** — IF an observability output would expose data classified as sensitive in violation of SQLI data security policy THEN the System SHALL redact or suppress that output. → ERR-003
...
**ERR-002** — IF a record fails processing after retry exhaustion THEN the System SHALL route it to a dead-letter store for engineer inspection. → FR-003

**ERR-003** — IF an observability output would expose data classified as sensitive in violation of SQLI data security policy THEN the System SHALL redact or suppress that output. → FR-016
...
> ⚠️ ERR-003 mirrors SEC-004 to capture the operational error path of the data-leakage prevention obligation (substrate O6).

> ⚠️ Substrate defines no explicit error paths for the majority of FRs (FR-001, FR-004..FR-005, FR-007..FR-017). Coverage is intentionally minimal at this maturity level; expand in design phase.
...
**NFR-002**
- AC: After retry exhaustion, the failed record is present in the dead-letter store and absent from the live processing path. Retention and triage SLA: TBD.
...
**ERR-001**
- AC: A failure injected into a pull triggers automatic retry events; the pull either succeeds within the retry budget or is recorded as a terminal failure.

**ERR-002**
- AC: A record forced through retry exhaustion is found in the dead-letter store with sufficient diagnostic context.

**ERR-003**
- AC: An observability event whose payload would include sensitive data in violation of policy is emitted with the sensitive content redacted, or is suppressed entirely.
