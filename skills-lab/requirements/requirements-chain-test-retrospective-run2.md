...
- Parked tightening decisions from the session analysis. R2, a durable oracle for the two `[beh]` rows, deferred by decision. The mutation halves are already durable via the diffs, so the residual is only the ask-versus-infer and stop-versus-report decision events. Add a minimal goal-decision stamp to `bootstrapping-project` when that skill is next edited, not standalone. The transcript-excerpt option is rejected as theatre. R3, decided: MD-3 and MD-7 stay scoped to the CONCEPTS layer, made explicit in each row's Purpose. The CONCEPTS-to-spec leg is covered by chain-test-medium and MD-6, and the divergence-to-spec leg is the deferred recomposition check. R4, now resolved: the XC-3 escape is fixed (`e4a0e06`) and XC-3 was updated to prove the contained behaviour without a scaffold, and SM-3's goal-persistence finding was already handled by the skill re-eliciting, confirmed in run 2, so nothing remains to unmask.
