...
- A fresh session.
- The directory `skills-lab/outputs/test-wr-degraded` exists and is empty. Session cwd is that directory.
...
Confirm the Title equals the slug verbatim, list which sections rendered `N/A`, and confirm the SHALL line survived as a functional requirement. Note that auto-derived glossary warnings, if any, are the usual verification-pending kind, separate from the degradation Warnings this test targets.
